---
title: "Importmaps in NodeJS using custom loaders"
...
import importmap from '../../assets/importmaps.svg';

<Image src={importmap} alt="Importmaps" width={600} height={400} />
...
Import-maps are supported in most of the major browsers at the moment. You can polyfill it by using [es-modue-shims](https://github.com/guybedford/es-module-shims) for older versions of browsers. You can read more about `import-map` in this detailed blogposts by mozilla team
...
<iframe
  ...
  title="Import maps browser support"
/>
...
However, NodeJS users need not despair just yet. NodeJS is experimenting with loading
modules from external URLs behind a flag. NodeJS custom loaders are also a powerful
feature in themselves, offering an elegant API to hook into Node's resolution algorithm
and load content on demand. Let’s put together a custom NodeJS loader using the

<a href="https://github.com/jspm/import-map">@jspm/import-map</a>
package and run an express-based server for serving some random facts.
...
import CSBScreenshot from "../../assets/csb-screenshot.png";

<Image
  alt="Running ImportMaps inside Nodejs using Codesandbox"
  src={CSBScreenshot}
  style={{ width: "100%" }}
/>
